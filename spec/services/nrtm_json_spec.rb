RSpec.describe NrtmJson do
  ...
  let(:tournament) { create(:tournament, name: 'Some Tournament', user: to, slug: 'SLUG', date: '2017-01-01', swiss_format: swiss_format) }
  ...
  describe '#data' do
    ...
    context 'with single-sided tournament' do
      ...
      before do
        round.pairings << create(:pairing, player1: jack, player2: jill, table_number: 1, score1_runner: 3, score2_corp: 0, side: :player1_is_runner)
        round.pairings << create(:pairing, player1: hansel, player2: gretel, table_number: 2, score1_corp: 3, score2_runner: 0, side: :player1_is_corp)
        round.pairings << create(:pairing, player1: snap, player2: crackle, table_number: 3, score1_corp: 1, score2_runner: 1, side: :player1_is_corp, intentional_draw: true)
