...
RSpec.describe SelfReporting do
  describe '#self_report_allowed' do
    let(:none_empty_self_report) do
      {
        'score_1' => 1,
        'score_2' => 1
      }
    ...
    it 'returns false if current user is nil' do
      expect(described_class.self_report_allowed(
               nil,
               nil,
               player_1_with_id,
               player_2_with_id
    ...
    it 'returns true if current user 1 is in pairing' do
      expect(described_class.self_report_allowed(alice,
                                                 nil,
                                                 player_1_with_id&.dig('user_id'),
                                                 player_2_with_id&.dig('user_id'))).to be true
    end

    it 'returns true if current user 2 is in pairing' do
      expect(described_class.self_report_allowed(bob,
                                                 nil,
                                                 player_1_with_id&.dig('user_id'),
                                                 player_2_with_id&.dig('user_id'))).to be true
    end
    ...
    it 'returns false if self_report exists' do
      expect(described_class.self_report_allowed(alice,
                                                 none_empty_self_report,
                                                 player_1_with_id&.dig('user_id'),
                                                 player_2_with_id&.dig('user_id'))).to be false
    end

    it 'returns false if current user is not part of the pairing' do
      expect(described_class.self_report_allowed(charlie,
                                                 nil,
                                                 player_1_with_id&.dig('user_id'),
                                                 player_2_with_id&.dig('user_id'))).to be false
    end
